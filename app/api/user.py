...
from ..utils import send_confirm, response_with_status, make_response, auto_commit_db, error_response
from ..db import db
...
def register():
    ...
    try:
        db.session.add(user)
        db.session.commit()
        send_confirm(path=data["path"], username=data["username"], forget=False, token=token,
